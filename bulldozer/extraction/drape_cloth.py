...
def allocate_dezoom_dtm(dezoom_shape: Tuple[int, ...], eomanager: eom.EOContextManager) -> str:
    """
    Create dezoomed dtm at dezoom shape.
    """
...
def upsample(
    ...
    next_dtm_key = allocate_dezoom_dtm(dezoom_shape=next_shape, eomanager=eomanager)
...
def drape_cloth_profiles(input_profiles: List[Dict[int, str]], params: Dict[float, int]) -> Dict[int, str]:
    # pylint: disable=unused-argument
...
def drape_cloth(
    ...
    dtm_key = allocate_dezoom_dtm(dezoom_shape=init_dtm_shape, eomanager=eomanager)
    ...
    while level >= 0:
        ...
        current_dezoom_profile: dict = downsample_profile(
            profile=eomanager.get_profile(key=filled_dsm_key), factor=2**level
        )
        ...
        ground_mask_key_memview = eomanager.create_memview(
            key=ground_mask_key,
