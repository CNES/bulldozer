...
def filled_dsm_profile(input_profiles: List[Dict[str, Any]], params: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
...
def fill_dsm_method(
    ...
    if filter_parameters["use_bordernodata_mask"] is True:
        # the input_buffers[0] corresponds to the input DSM raster
        # input_buffers[1] corresponds to the bordernodata mask
        dsm = fill_process.iterative_filling(
            dsm_strip=input_buffers[0][0, :, :],
...
def fill_dsm(
    ...
    # overides the value with 2 to ensure a dezoom during the filling process
    dezoom_factor = int(
        np.max([2, np.floor(regular_parameters["filling_iterations"] * (2 - np.sqrt(2)))])
    ...
    BulldozerLogger.log(
        ...
        + f"filling_iterations={regular_parameters['filling_iterations']} / "
    ...
    while has_nodata and 0 <= dezoom_level <= nb_max_level:
        ...
        if dezoom_level == 0:  # When level is 0
            ...
            [dsm_key] = eoexe.n_images_to_m_images_filter(
                inputs=[dsm_key, border_nodata_key],
            ...
            if dev_mode:
                ...
                profile["nodata"] = nodata
                if downsample:
                    filled_dsm_1stpass_path = os.path.join(dev_dir, "filled_dsm_downsample_level_0.tif")
                else:
                    # Upsample case
                    filled_dsm_1stpass_path = os.path.join(dev_dir, "filled_dsm_upsample_level_0.tif")
            ...
            # Downsampling the DSM to fill the large nodata areas.
            ...
            downsampled_profile = downsample_profile(
                profile=eomanager.get_profile(key=dsm_key), factor=dezoom_factor**dezoom_level
            )
            ...
            if downsample:
                ...
                BulldozerLogger.log(
                    ...
                    + f"default_tile_size={int(math.sqrt((filled_dsm.shape[0] * filled_dsm.shape[1]) // eomanager.nb_workers))}",
                    logging.DEBUG,
                )
                ...
                regular_parameters["filling_iterations"] = int(
                    np.floor(
            ...
            if np.ceil(dsm_resolution * dezoom_factor**dezoom_level) > math.sqrt(
                (filled_dsm.shape[0] * filled_dsm.shape[1]) // eomanager.nb_workers
            ):
                # TODO HOTFIX to remove: until we change eoscale we have to compute the tile size manually
            ...
            [downsampled_filled_dsm_key] = eoexe.n_images_to_m_images_filter(
                inputs=[downsampled_filled_dsm_key],
                image_filter=fill_dsm_method,
                filter_parameters=regular_parameters,
                generate_output_profiles=filled_dsm_profile,
                context_manager=eomanager,
            ...
            if dev_mode:
                if downsample:
                    filled_dsm_path = os.path.join(
                        dev_dir, "filled_dsm_downsampled_level_" + str(dezoom_level) + ".tif"
                    )
                else:
                    # Upsample case
                    filled_dsm_path = os.path.join(dev_dir, "filled_dsm_upsampled_level_" + str(dezoom_level) + ".tif")
