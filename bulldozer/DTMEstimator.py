...
class DTMEstimator(object):
    ...
    def recursive_fit_dtm(self, dtm, dsm, step=1, level=0):
        ...
        if numpy.min(dtm.shape) > 100:
            print("Go a level deeper in the image pyramid.")
            ...
            # plt.figure()
            # plt.subplot(121)
            # plt.imshow(sm_dsm, vmin=numpy.nanmin(sm_dsm), vmax=numpy.nanmax(sm_dsm), cmap="gray")
            # plt.title("DSM level=" + str(level))
            # plt.subplot(122)
            # plt.imshow(sm_dtm, vmin=numpy.nanmin(sm_dsm), vmax=numpy.nanmax(sm_dsm), cmap="gray")
            # plt.title("DTM level=" + str(level))
            # plt.show()
        ...
        print("Reached min size {}. Revisit levels until level 0.".format(dtm.shape))
    ...
    def drape_cloth(self, dtm, dsm, step=1, num_outer_iter=10):
        ...
        # # Display
        # plt.figure()
        # plt.imshow(dtm, vmin=numpy.nanmin(dsm), vmax=numpy.nanmax(dsm), cmap="gray")
        # plt.title("Cloth DTM step=" + str(step) + ", num_outer_iter=" + str(num_outer_iter))
        # plt.show()
