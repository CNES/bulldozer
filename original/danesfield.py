...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python danesfield.py [input dsm] [output dtm]")
    else:
        with rasterio.open(sys.argv[1], "r") as dsmDataset:
            nodata = dsmDataset.nodata
            profile = dsmDataset.profile
            dtmEstimator = DTMEstimator(nodata_val=nodata)
            dtm = dtmEstimator.fit_dtm(dsmDataset.read(1))
            with rasterio.open(sys.argv[2], "w", **profile) as dtmDataset:
                dtmDataset.write(dtm, 1)